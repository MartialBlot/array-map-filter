...
function getCirclesProperties(radiuses) {
  return radiuses.map ( radiuses => [radiuses["radius"] =  `radius: ${radiuses}` , radiuses["circumference"] = `circumference: ${radiuses*Math.PI*2}` , radiuses["surface"] = `surface: ${radiuses*radiuses*Math.PI}`])
}
