...
function getMoviesFreshness(movies) {
  let label ='';
  return movies = movies.map(movies => (movies.rating<60) ? movies.push(`${movies} ${label}: 'rotten'`):(movies.rating>75) ? movies.push(`${label}: certified 'fresh'`) : movies.push(`${label}: 'fresh'`))
}
