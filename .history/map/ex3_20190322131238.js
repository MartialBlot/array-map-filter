...
function getMoviesFreshness(movies) {
  let label ='';
  let result = [];
  return result = movies.map(movies => (movies.rating<60) ? result.push(`${movies} ${label}: 'rotten'`):(movies.rating>75) ? result.push(`${label}: certified 'fresh'`) : result.push(`${label}: 'fresh'`))
}
